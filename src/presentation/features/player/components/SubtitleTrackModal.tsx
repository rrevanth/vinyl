...
export const SubtitleTrackModal = memo<SubtitleTrackModalProps>(
  observer(({ visible, onClose, onSelectTrack }) => {
    ...
    const renderTrackItem = (track: SubtitleTrack | null) => {
      const isSelected = track ? track.id === selectedTrackId : selectedTrackId === null
      const isOff = track === null
      ...
      return (
        <Pressable
          ...
          accessibilityLabel={isOff ? t('player.off') : track?.name}
          accessibilityState={{ selected: isSelected }}
        >
          <View style={styles.trackInfo}>
            <Text style={[styles.trackTitle, isSelected && styles.trackTitleSelected]}>
              {isOff ? t('player.off') : track?.name}
            </Text>
            {track?.language ? (
    ...
    return (
      <Modal
        ...
        accessibilityLabel={t('player.subtitle_tracks')}
      >
        <Pressable
        ...
        >
          ...
          >
            <View style={styles.header}>
              ...
              <Text style={styles.title}>{t('player.subtitle_tracks')}</Text>
            </View>

            <ScrollView style={styles.trackList} bounces={false}>
              {subtitleTracks.length === 0 ? (
                <View style={styles.emptyState}>
                  <Text style={styles.emptyText}>{t('player.no_subtitle_tracks')}</Text>
                </View>
              ) : (
                <>
                  {renderTrackItem(null)}
                  {subtitleTracks.map((track) => renderTrackItem(track))}
                </>
              )}
            </ScrollView>
          </Pressable>
