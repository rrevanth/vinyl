...
export const BrightnessOverlay: React.FC<BrightnessOverlayProps> = ({
  ...
  const getBrightnessIcon = (): keyof typeof MaterialIcons.glyphMap => {
    if (brightness < 0.2) return 'brightness-low'
    if (brightness < 0.5) return 'brightness-medium'
    if (brightness < 0.8) return 'brightness-high'
    return 'brightness-auto'
  }

  const isLow = brightness < 0.2
  const iconColor = isLow ? '#FFD700' : '#FFFFFF'
  const fillColor = isLow ? '#FFD700' : '#FFA500'
